...
import React, { useState, useEffect, useCallback, useMemo, Suspense } from 'react'; // Added Suspense
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-zinc-900 dark:text-white">
                {categoryInfo ? (
                  <><span className="mr-2">{categoryInfo.icon}</span>{categoryInfo.name}</>
                ) : ('Gallery')}
              </h1>
              <p className="text-zinc-600 dark:text-zinc-400 mt-1">
                {categoryInfo ? categoryInfo.description : `Browse ${images.length}+ beautiful anime images`}
              </p>
            </div>
            <button onClick={() => setShowFilters(!showFilters)} className="md:hidden btn-secondary">
              Filters
            </button>
          </div>
          <div className="hidden md:block">
            <CategoryTabs selectedCategory={selectedCategory} onCategoryChange={handleCategoryChange} categories={filteredCategories} />
          </div>
        </div>
        <div className="flex gap-8">
          <div className="hidden lg:block w-64 flex-shrink-0">
            <div className="sticky top-24 space-y-6">
                <div className="bg-zinc-50 dark:bg-zinc-900 rounded-2xl p-4">
                    <h3 className="font-semibold text-zinc-900 dark:text-white mb-3">🎭 Filter by Mood</h3>
                    <div className="space-y-2">
                        {(['happy', 'sad', 'romantic', 'playful', 'aggressive', 'calm', 'excited', 'cute'] as Mood[]).map((mood) => (
                            <button
                                ...
                                className={cn('w-full text-left px-3 py-2 rounded-lg text-sm transition-colors', selectedMood === mood ? 'bg-pink-500 text-white' : 'hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300')}
                            >
          ...
          <div className="flex-1 min-w-0">
            <ImageGallery images={images} loading={loading} hasMore={hasMore} onLoadMore={handleLoadMore} columns={4} emptyMessage="No images found." />
          </div>
        </div>
      </div>
    </div>
